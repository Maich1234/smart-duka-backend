...
A production-ready REST API for the DuQana Point‑of‑Sale system. Built with Node.js, Express, MongoDB, and JWT authentication.
...
| PUBLIC_WEB_URL | Base URL of the deployed public web app; used to build the QR code link on receipts | `https://duqana.app` |
...
| MPESA_CALLBACK_URL | Public HTTPS URL Safaricom POSTs STK Push results to (do **not** append anything — the server appends `MPESA_CALLBACK_SECRET` itself) | `https://api.dukana.co.ke/api/v1/mpesa/callback` |
...
| MPESA_CALLBACK_SECRET | Shared secret the server appends as the final path segment of every M-Pesa callback URL it hands Safaricom (STK, reversal, and the subscription/seat-payment callback) — Daraja has no callback-signing mechanism, so without this anyone who learns a transaction's reference could POST a forged success result | `a-long-random-string` |

> **Paystack, like M-Pesa, is configured from the super-admin Platform Config page — not env vars.** Secret/public key and the enabled toggle live on `PlatformConfig.paystack` (encrypted at rest the same way M-Pesa's Daraja credentials are). Separately, the webhook URL itself is set once in the **Paystack dashboard** (Settings → API Keys & Webhooks), pointing at `https://api.dukana.co.ke/api/v1/subscriptions/paystack/webhook` — there's no per-request `callbackUrl` param like M-Pesa's STK push. Without it, payments still work via the owner-facing recheck button and the daily reconciliation cron, just without the instant confirmation.
...
> **`SMTP_FROM` quoting.** In a `.env` file, `SMTP_FROM="DuQana" <noreply@example.com>` parses to just `DuQana` — the quoted display name terminates the value and the address is dropped. Nodemailer then has no address to use and sends `MAIL FROM:<>`, a null return-path that receivers treat as a bounce and spam-file or reject. Wrap the whole value in single quotes (`SMTP_FROM='"DuQana" <noreply@example.com>'`) or leave it unquoted. Dashboard-set env vars (Vercel) aren't parsed this way, so paste the plain `"DuQana" <noreply@example.com>` form there. `sendEmail` defends against the mistake by falling back to `SMTP_USER` as the address, but fix the variable rather than relying on that.
...
Use Postman or any API client. Example collection: [DuQana API.postman_collection.json](./postman/collection.json)
